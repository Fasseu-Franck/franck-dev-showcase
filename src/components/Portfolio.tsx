...
const Portfolio = () => {
  ...
  return (
    <section id="portfolio" className="py-20 md:py-32 bg-background relative overflow-hidden">
      {/* Background Ice Effects */}
      <div className="absolute inset-0 gradient-glow opacity-10"></div>
        ...
        <div className="max-w-6xl mx-auto relative z-10">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
            {projects.map((project, index) => (
              <div
                key={project.title}
                className="glass-ice rounded-2xl overflow-hidden hover:shadow-glow transition-all duration-300 hover:scale-105 animate-fade-in-up group liquid-border"
                style={{ animationDelay: `${index * 0.1}s` }}
              >
                {/* Project Header */}
                <div className={`h-32 ${project.gradient} relative overflow-hidden shadow-glow`}>
